...
function resolveRegionCodes() {
  try {
    const values = intlWithSupportedValuesOf.supportedValuesOf?.('region');
    if (values && values.length > 0) {
      return values.filter((code) => /^[A-Z]{2}$/.test(code));
    }
  } catch {
    // Fallback for runtimes that do not support 'region' as key.
  }
...
export const COUNTRY_OPTIONS = regionCodes
  .map((code) => ({
    code,
...
export function getCountryLabel(value: string) {
  const entry = COUNTRY_OPTIONS.find((country) => country.code === value);
